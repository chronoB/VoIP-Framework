...
void CPortaudioPlus::init_InSound(void)
{
	...
	if(err != paNoError)
	{
		printf("InSound nach StartStream");
		pa_error(-3);
		return;
	}
...
void CPortaudioPlus::init_OutSound(void)
{
	...
	if (err != paNoError)
	{
		printf("OutSound nach StartStream");
		pa_error(-6);
		return;
	}
	printf("Stream Started! \n");
...
void CPortaudioPlus::init_InOutSound(void)
{
	...
	if (err != paNoError)
	{
		printf("InOutSound nach OpenStream");
		pa_error(-8);
		return;
	}
	printf("Stream open!\n");
	...
	if (err != paNoError)
	{
		printf("InOutSound nach StartStream");
		pa_error(-9);
		return;
	}
	printf("Stream Started! \n");
...
void CPortaudioPlus::set_device(const char* devName,const char* ApiName, int audioMode)
{
	...
	{
		printf("ERROR: Input Device nicht gefunden!\n");
		pa_error(-1);
	}
	if (deviceIDout == PaNoDevice && audioMode == 2)
	{
		printf("ERROR: Output Device nicht gefunden!\n");
		pa_error(-1);
	}	
}

// gen stereo output blog from mono input
...
// gen stereo output blog from two mono input blocks
